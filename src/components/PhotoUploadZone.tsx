...
export const PhotoUploadZone: React.FC<PhotoUploadZoneProps> = ({
  ...
  const processFiles = (fileList: FileList | File[]) => {
    ...
    if (photos.length + files.length > MAX_PHOTOS) {
      onErrorChange(`Maximum of ${MAX_PHOTOS} photos allowed per item.`);
      return;
    }
    ...
    for (const file of files) {
      if (!ALLOWED_TYPES.includes(file.type)) {
        onErrorChange(`"${file.name}" is unsupported. Please upload JPG, PNG, or WebP.`);
        return;
      }
      if (file.size > MAX_SIZE_BYTES) {
        const sizeMB = (file.size / (1024 * 1024)).toFixed(1);
        onErrorChange(`"${file.name}" is ${sizeMB}MB (max allowed is 10MB).`);
        return;
      }
  ...
  return (
    <div className="w-full space-y-3">
      ...
      <div
        ...
        className={`relative w-full rounded-2xl border-2 border-dashed p-6 sm:p-7 cursor-pointer transition-all duration-200 ease-out flex flex-col items-center justify-center text-center bg-white shadow-warm ${
          isDragOver
            ? "border-[#E8623D] bg-[#FDF2EF] shadow-warmHover scale-[1.005]"
            : "border-[#E4DEC9] hover:border-[#E8623D] hover:bg-[#FAF7F2] hover:shadow-warmHover"
        }`}
      >
        <div className="w-11 h-11 rounded-full bg-[#FDF2EF] border border-[#F5C7B8] flex items-center justify-center text-[#E8623D] mb-2.5">
          <Camera className="w-5 h-5" />
        </div>

        <h3 className="font-heading font-semibold text-base text-[#18181B] tracking-tight">
          {photos.length === 0 ? "Drop item photos here or click to browse" : "Add more angles or label shots"}
        </h3>
        <p className="text-xs text-[#71717A] mt-0.5">
          Front, back, tags, and hardware details (up to 4 photos)
        </p>

        {/* Counter Badge */}
        <div className="mt-3 inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full text-xs font-medium bg-[#F5F2EB] text-[#52525B]">
      ...
      {photos.length > 0 && (
        <div className="space-y-1.5 pt-1">
          ...
          <div className="flex items-center gap-3 overflow-x-auto pb-2 pt-0.5 scrollbar-thin">
